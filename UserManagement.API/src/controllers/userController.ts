...
export class UserController {
	...
	public async createUser(req: Request, res: Response): Promise<void>{
		try {
			...
			const {
				error.value: payload } = validateUser(req.body);
...
public async getAllUserByID(req: Request, res: Response): Promise<void>{
	try{
...
public async updateUser(req: Request, res: Response): Promise<void>{
	try{
		...
		const user: IUser | null = await User.findByIdAndUpdate(req.params.id,userData, 
			{new: true}
			//This option returns the modified document rather that the original
			);
...
//Delete user 
//Handles DELETE request to remove a user from the database 
...
export class UserController {
  ...
  public async getAllUsers(req: Request, res: Response): Promise<void> {
    try {
      // Fetch all users from the database, excluding sensitive password field
      const users: IUser[] = await User.find().select("-password");
      res.json(users);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
  ...
  public async getUserById(req: Request, res: Response): Promise<void> {
    try {
      // Attempt to find the user by ID, excluding password field
      const user: IUser | null = await User.findById(req.params.id).select("-password");
