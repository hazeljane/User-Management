...
export function corsHandler(req: Request, res: Response, next: NextFunction ){
	//Allow request from the origin specified in the request header
	const origin = req.header("header");
	if (origin){
	...
	if (req.method === "OPTIONS") {
		...
		res.header("Access-Control-Allow-Methods",
			"GET, HOST, PUT, PATCH, DELETE, OPTIONS");
		//Respond to preflight request with 204 No Content status return 
		res.sendStatus(204);
	}
